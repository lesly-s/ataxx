...
class Board {
    ...
    void clear() {
        ...
        for (char c = 'a'; c <= 'g'; c++) {
            for (char r = '1'; r <= '7'; r++) {
                if (c == 'a' && r == '1' || c == 'g' && r == '7') {
                    unrecordedSet(index(c, r), BLUE);
                } else if (c == 'a' && r == '7' || c == 'g' && r == '1')  {
                    unrecordedSet(index(c, r), RED);
                } else {
                    unrecordedSet(index(c, r), EMPTY);
                }
            }
        }
    ...
    boolean legalMove(Move move) {
        if (move == null || getWinner() != null) {
            return false;
        }
        if (move.isPass()) {
            return !canMove(_whoseMove);
        }
        PieceColor pieceMove = get(move.fromIndex());
        if (pieceMove != _whoseMove) {
            return false;
        }
            ...
            for (char r = '1'; r <= '7'; r++) {
                for (char y = (char) (c - 2); y <= (char) (c + 2); y++) {
                    for (char x = (char) (r - 2); x <= (char) (r + 2); x++) {
                        if (get(index(c, r)).equals(who)) {
                            if (get(index(y, x)).equals(EMPTY)) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        PieceColor color = get(move.fromIndex());
        int rowDiff = move.row1() - move.row0();
        int colDiff = move.col1() - move.col0();

        if (Math.abs(rowDiff) == 2 || Math.abs(colDiff) == 2) {
            set(move.toIndex(), color);
            makeMoveFlip(move);
            set(move.fromIndex(), EMPTY);
            _numJumps = _numJumps + 1;
        } else if (Math.abs(rowDiff) == 1 || Math.abs(colDiff) == 1) {
            set(move.toIndex(), color);
            incrPieces(get(move.toIndex()), 1);
            makeMoveFlip(move);
    ...
    /** Helper method that checks the winner of the game. */
    void checkWinner() {
        PieceColor opponent = _whoseMove.opposite();
        if (!canMove(whoseMove()) && !canMove(opponent)) {
        ...
        if (_totalOpen == 0 || _numJumps == JUMP_LIMIT) {
            if (bluePieces() > redPieces()) {
                _winner = BLUE;
            }
            if (redPieces() > bluePieces()) {
    ...
    /** Checks if I should flip my neighbors based on move MOVE. */
    void makeMoveFlip(Move move) {
        int leftNeighbor = neighbor(move.toIndex(), -1, 0);
        int rightNeighbor = neighbor(move.toIndex(), 1, 0);
        int topNeighbor = neighbor(move.toIndex(), 0, 1);
        int bottNeighbor = neighbor(move.toIndex(), 0, -1);
        int topLeft = neighbor(move.toIndex(), -1, 1);
        int topRight = neighbor(move.toIndex(), 1, 1);
        int bottLeft = neighbor(move.toIndex(), -1, -1);
        int bottRight = neighbor(move.toIndex(), 1, -1);

        flippy(leftNeighbor, move);
        flippy(rightNeighbor, move);
        flippy(topNeighbor, move);
        flippy(bottNeighbor, move);
        flippy(topLeft, move);
        flippy(topRight, move);
        flippy(bottLeft, move);
        flippy(bottRight, move);
    }

    /** Flips PieceColor based on my current move MOVE, neighbor NEIGHBOR. */
    void flippy(int neighbor, Move move) {
        PieceColor nColor = get(neighbor);
        PieceColor frIndx = get(move.fromIndex());
        PieceColor toIndx = get(move.toIndex());
        if (!nColor.equals(BLOCKED) && !nColor.equals(frIndx)) {
            if (toIndx == RED && nColor.equals(BLUE)) {
                incrPieces(BLUE, -1);
                set(neighbor, toIndx);
                incrPieces(toIndx, 1);
            }
            if (toIndx == BLUE && nColor.equals(RED)) {
                incrPieces(RED, -1);
                set(neighbor, toIndx);
                incrPieces(toIndx, 1);
            }
    ...
    boolean legalBlock(char c, char r) {
        char colOpp = (char) ('g' - c + 'a');
        char rowOpp = (char) ('7' - r + '1');
        ...
        if (allMoves().size() == 0) {
            if (get(c, r).equals(EMPTY)) {
                if (c == 'd' && r == '4') {
                    ...
                    return get(c, rowOpp).equals(EMPTY);
                } else if (r == '4') {
                    return get(colOpp, r).equals(EMPTY);
                } else {
                    return get(colOpp, rowOpp).equals(EMPTY);
                }
            }
        }
        return false;
        ...
        if (get(c, r).equals(EMPTY)) {
            if (c != 'a' && r != '1' || r != '7') {
                if (c != 'g' && r != '1' || r != '7') {
                    char colOpp = (char) ('g' - c + 'a');
                    char rowOpp = (char) ('7' - r + '1');
                    if (get(colOpp, r) != BLOCKED) {
                        unrecordedSet(colOpp, r, BLOCKED);
                        _totalOpen = _totalOpen - 1;
                    }
                    if (get(c, r) != BLOCKED) {
                    ...
                    if (get(colOpp, rowOpp) != BLOCKED) {
                        unrecordedSet(colOpp, rowOpp, BLOCKED);
                        _totalOpen = _totalOpen - 1;
                    }
                    if (get(c, rowOpp) != BLOCKED) {
                        unrecordedSet(c, rowOpp, BLOCKED);
                        _totalOpen = _totalOpen - 1;
                    }
                }
